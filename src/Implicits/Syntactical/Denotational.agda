...
module RewriteContext where
  ...
  postulate #ival : ∀ {ν n} {K : Ktx ν n} → (a : Binding ν) → K# K → K# (a ∷K K)
...
⟦_,_⟧ (implicit_in'_ {a = a} t e) m = (F.λ' ⟦ a ⟧pt ⟦ e , #ival (val a) m ⟧) F.· ⟦ t , m ⟧
⟦_,_⟧ (implicit_⇒_in'_ a t e) m = ?
...
⟦⟧-preserves-tp (implicit_⇒_in'_ {b = b} a wt-e₁ wt-e₂) m 
  with ⟦⟧-preserves-tp wt-e₁ (#ival (val a) m) | ⟦⟧-preserves-tp wt-e₂ (#ival (rule a b) m)
⟦⟧-preserves-tp (implicit_⇒_in'_ a wt-e₁ wt-e₂) m | ih | y = ?
