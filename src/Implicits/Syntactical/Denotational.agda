...
⟦ implicit_in'_ (val {a = a} x) e , m ⟧ =
  (F.λ' ⟦ a ⟧tp ⟦ e , #ival (⊢wfi-to-implicit $ val x) m ⟧) F.· ⟦ x , m ⟧
⟦ implicit (rule {a = a} x ft) in' e , m ⟧ =
  (F.λ' ⟦ a ⟧tp ⟦ e , #ival (⊢wfi-to-implicit $ rule x ft) m ⟧) F.· ⟦ x , m ⟧
...
module Lemmas where
  ...
  ⟦function⟧⟶function : ∀ {ν} {a : Type ν} → IsFunction a → F.IsFunction ⟦ a ⟧tp
  ...
  lambda-codomain⋆⟦⟧ : ∀ {ν} {a : Type ν} → (r : IsFunction a) →
                     ⟦ codomain r ⟧tp ≡ F.codomain (⟦function⟧⟶function r)
  ...
  lambda-domain⋆⟦⟧ : ∀ {ν} {a : Type ν} → (r : IsFunction a) →
                   ⟦ domain r ⟧tp ≡ F.domain (⟦function⟧⟶function r)
  ...
  poly-· : ∀ {ν n} {a} {Γ : F.Ctx ν n} {f t} →
           (ρ : IsFunction a) → Γ F.⊢ f ∈ ⟦ a ⟧tp → Γ F.⊢ t ∈ ⟦ domain ρ ⟧tp →
...
private
  ...
  inst-ρ : ∀ {ν n} {K : Ktx ν n} {r a t} → K# K →
           ρ⟨ K , r ⟩↝ a → ⟦ K ⟧ctx F.⊢ t ∈ ⟦ totype r ⟧tp → ∃ λ t' → ⟦ K ⟧ctx F.⊢ t' ∈ ⟦ a ⟧tp
  inst-ρ _ (by-value r) ⊢a = , ⊢a
  inst-ρ m (by-implication {r = r} Δ↝arg) ⊢r = poly-· r ⊢r (proj₂ $ ⟦ Δ↝arg , m ⟧i)
...
⟦⟧-preserves-tp (implicit val {a = a} ⊢t in' wt-e₂) m | ih₁ | ih₂ = (F.λ' ⟦ a ⟧tp ih₂) F.· ih₁
⟦⟧-preserves-tp (implicit rule {a = a} ⊢t ft in' wt-e₂) m
  with ⟦⟧-preserves-tp ⊢t m | ⟦⟧-preserves-tp wt-e₂ (#ival (⊢wfi-to-implicit $ rule ⊢t ft) m)
⟦⟧-preserves-tp (implicit rule {a = a} ⊢t _ in' wt-e₂) m | ih₁ | ih₂ = (F.λ' ⟦ a ⟧tp ih₂) F.· ih₁
