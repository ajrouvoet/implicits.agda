...
_,_amatch_ : ∀ {m ν} → (MetaType m ν) → (ρs : MICtx m ν) → (MetaSimpleType m ν) → Maybe (ICtx ν)
(a ⇒ b) , ρs amatch τ = b , a List.∷ ρs amatch τ
∀' x , ρs amatch τ = open-meta (∀' x) , (List.map meta-weaken ρs) amatch smeta-weaken τ
(simpl x) , ρs amatch τ with mgu (simpl x) (simpl τ)
...
r match a = (to-meta {zero} r) , List.[] amatch (to-smeta a)
...
module relational where
  data _,_⊢_matches_ {ν} (ρs : List (Type ν)) : Fin (suc ν) → Type ν → SimpleType ν → Set where
    ...
    mtc-simp : ∀ {α a b} → Unifiable (simpl a) (simpl b) → ρs , α ⊢ (simpl a) matches b
  ...
  gather (m1-head x) = gather' x
    where
        ...
        gather' {ρs = ρs} {α = α} (mtc-simp {a = a} {b = b} u) =
            List.map (substitute α {simpl a} {simpl b} u) ρs
