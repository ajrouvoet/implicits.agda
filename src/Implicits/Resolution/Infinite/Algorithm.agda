...
module Completeness where
  ...
  -- We actually have a 'minimal' completeness result at most, because the rules allow
  -- for many derivations, while the algorithm will only find *one*.

  -- for every finite resolution derivation, the algorithm succeeds or diverges
  -- postulate completeness : ∀ {ν} {Δ : ICtx ν} {r} → Δ ⊢ᵣ r → ???

  completeness : ∀ {ν} (Δ : ICtx ν) r → Δ ⊢ᵣ r → All (λ{ true → ⊤; false → ⊥ }) (resolve Δ r)
